...
?&gt;</code></pre>";
        ...
        public function subnav_context($action) {
            ...
            foreach (Config::current()->enabled_feathers as $index => $feather) {
                ...
                $subnav["write"]["write_post&feather=".$feather] =
                    array("title" => __($info["name"], $feather),
            ...
            foreach (array_keys($subnav["manage"]) as $manage)
                $pages["manage"] = array_merge($pages["manage"],
                    array($manage, preg_replace_callback("/manage_(.+)/",
                        function($m) {
        ...
        public function display($action, $context = array(), $title = "") {
            ...
            $filePath = ADMIN_THEMES_DIR."/%s/pages/".$action.".twig";
                ...
                foreach (array(MODULES_DIR => $config->enabled_modules,
                    ...
                    foreach ($try as $extension)
                        if (file_exists($path."/".$extension."/pages/admin/".$action.".twig")) {
                            ...
                            $this->twig->addTemplatePath($path."/".$extension."/pages/admin/");
                            $file = $action.".twig";
                        }
            ...
            try {
                return $this->twig->display($file, $this->context);
            } catch (Exception $e) {
